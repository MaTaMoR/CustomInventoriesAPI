...
public final class ItemReflections {
    ...
    private static final MethodInvoker TO_METHOD = Reflections.getMethod(CRAFT_ITEM_STACK_CLASS, "asNMSCopy", ItemStack.class);
    private static final MethodInvoker FROM_METHOD = Reflections.getMethod(CRAFT_ITEM_STACK_CLASS, "asCraftMirror", CRAFT_ITEM_STACK_CLASS);

    private static final MethodInvoker SET_COMPOUND_METHOD = Reflections.getMethod(NMS_CLASS, "setTag", NBT_COMPOUND_CLASS);
    private static final MethodInvoker GET_COMPOUND_METHOD = Reflections.getMethod(NMS_CLASS, "getTag");

    private static final MethodInvoker SET_METHOD = Reflections.getMethod(NBT_COMPOUND_CLASS, "set", String.class, NBT_BASE_CLASS);
    private static final MethodInvoker GET_METHOD = Reflections.getMethod(NBT_COMPOUND_CLASS, "get", String.class);
    private static final MethodInvoker NBT_HAS_KEY_METHOD = Reflections.getMethod(NBT_COMPOUND_CLASS, "hasKey", String.class);
    private static final MethodInvoker NBT_KEYS_METHOD = Reflections.getMethod(NBT_COMPOUND_CLASS, "c");

    private static final FieldAccessor<GameProfile> PROFILE_FIELD = Reflections.getField(Reflections.getCraftBukkitClass("inventory.CraftMetaSkull"), "profile", GameProfile.class);
    ...
    public static ItemStack setNBTTag(ItemStack itemStack, String key, NBTTag value) {
        ...
        if(tagCompound == null) tagCompound = newNBTTagCompound();
    ...
    public static NBTTag getNBTTag(ItemStack itemStack, String key) {
        ...
        if(craftItemStack == null) throw new NullPointerException("NMSItem is null");
    ...
    public static ItemStack setContent(ItemStack itemStack, Map<String, NBTTag> content) {
        ...
        Object nbtTag = getNBTTagCompound(craftItemStack);
        if(nbtTag == null) nbtTag = newNBTTagCompound();
    ...
    public static void setTexture(ItemStack itemStack, SkullData data) {
        if(itemStack.getItemMeta() instanceof SkullMeta) {
            ...
            } else if(data.getType() == SkullDataType.URL) {
    ...
    public static String getTexture(ItemStack itemStack) {
        if(itemStack.getItemMeta() instanceof SkullMeta) {
            ...
            if(profile == null || profile.getProperties().get("textures") == null) return null;
